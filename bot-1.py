...
NVIDIA_API_KEY = os.environ.get("NVIDIA_API_KEY")  # key từ build.nvidia.com

# ===== DANH SACH MODEL =====
# provider: "openrouter" | "github" | "nvidia"
models = [
    # OpenRouter free
    ...
    # NVIDIA NIM
    {"name": "Gemma 4 31B (NIM)", "id": "google/gemma-4-31b-it", "provider": "nvidia"},
]
...
def goi_nvidia_nim(model_id, messages):
    try:
        response = requests.post(
            ...
            timeout=60  # NIM có thể chậm hơn, tăng timeout
        )
        data = response.json()
        if "choices" in data:
            return data["choices"][0]["message"]["content"]
        elif "error" in data:
            return parse_error(data, "Loi NVIDIA NIM")
        return "Khong co phan hoi (status: " + str(response.status_code) + ")"
    except requests.exceptions.Timeout:
        return "Loi: Qua thoi gian cho (timeout)"
    except Exception as e:
        return "Loi ket noi NVIDIA NIM: " + str(e)

def goi_ai(provider, model_id, messages):
    if provider == "github":
        return goi_github_models(model_id, messages)
    elif provider == "nvidia":
        return goi_nvidia_nim(model_id, messages)
    else:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Bot AI + GitHub\n\n"
        "CHAT AI:\n"
        "/model - Chon model (OpenRouter + GitHub + NVIDIA NIM)\n"
...
async def chon_model(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if row:
        keyboard.append(row)
    await update.message.reply_text(
        "Chon model AI:\n(OR = OpenRouter | GH = GitHub Models | NIM = NVIDIA NIM)",
...
async def xu_ly_chon_model(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    provider_label = provider_labels.get(model["provider"], "OpenRouter")
...
async def my_model(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_data:
        ...
        provider_label = provider_labels.get(d["provider"], "OpenRouter")
        await update.message.reply_text(
            "Model: " + d["model_name"] + "\nNguon: " + provider_label
        )
    else:
